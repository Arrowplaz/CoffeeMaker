...
public class APIRecipeController extends APIController {
    ...
    /**
     * REST API method for providing put access to the recipe model. It is used to update a current recipe
     * @param name the name of the recipe to replace
     * @param recipe the recipe to replace it with
     * @return ResponseEntity indicating if the recipe was updated
     */
    @PutMapping ( BASE_PATH + "/recipes/{name}")
    public ResponseEntity updateRecipe (@PathVariable ("name") final String name, @RequestBody final Recipe recipe) {
    	if ( null != service.findByName(name) ) {
    		Recipe r = service.findByName(name);
    	...
    	return new ResponseEntity( errorResponse( "Recipe with the name " + name + " does not exists" ),
                HttpStatus.CONFLICT );
